    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const wallet of existingWallets) {
            ...
            do {
                ...
                const existingCode = await queryRunner.query(`SELECT id FROM "wallets" WHERE "referralCode" = $1`, [referralCode]);
                if (existingCode.length === 0) {
                    isUnique = true;
                }
            } while (!isUnique);

            await queryRunner.query(`UPDATE "wallets" SET "referralCode" = $1 WHERE "id" = $2`, [referralCode, wallet.id]);
        }
