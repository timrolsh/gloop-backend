...
export class TransactionService {
    ...
    async create(createTransactionDto: CreateTransactionDto): Promise<Transaction> {
        ...
        if (!wallet) {
            throwCustomHttpException(`Wallet with address ${walletAddress} not found`, `Wallet with address ${walletAddress} not found`, HttpStatus.NOT_FOUND);
        }
    ...
    async getLiquidations(paginationDto: PaginationDto): Promise<ResultDto<LiquidationRequestDto[]>> {
        ...
        try {
            ...
            const result = new ResultDto(orderData, new ResponseMessage("Data retrieve successfully", "Data retrieve successfully"));
            result.paging = meta;
    ...
    private async populateCollateralData(assets: { name: string; address: string }[], walletData: Record<string, LiquidationRequestDto>, limit: number, offset: number) {
    ...
    private processCollateralResults(results: any[], asset: { name: string; address: string }, walletData: Record<string, LiquidationRequestDto>) {
    ...
    private async populateUsdcDebtData(walletData: Record<string, LiquidationRequestDto>, limit: number, offset: number, usdcAddress: string) {
    ...
    private processUsdcDebtResults(results: any[], walletData: Record<string, LiquidationRequestDto>) {
        for (const result of results) {
    ...
    private async getTotalWalletsCount(assets: { name: string; address: string }[], usdcAddress: string): Promise<number> {
