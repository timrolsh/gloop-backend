...
export class WatcherReferralService {
    ...
    private initializeProviderAndContract() {
        ...
        this.contract = new ethers.Contract(this.configService.get("crypto.gmPointContactAddress"), this.ABI, this.provider);
    ...
    private async checkNetworkConnection() {
        try {
            ...
            this.logger.log(`Connected to network (${this.configService.get("crypto.network")}): ${network.name} (${network.chainId})`);
            // this.watchOnline();
        } catch (error) {
