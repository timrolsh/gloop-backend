...
import { RequestReferralCodeDto } from "./dto/request-referral-code.dto";
...
export class WalletService {
    ...
    async getUserDashboard(walletId: string): Promise<ResultDto<DashboardResponseDto>> {
        ...
        const rankQuery = this.walletRepository
            ...
            .where(
                new Brackets((qb) => {
                    qb.where("wallet.totalEarnedPoints > :totalEarnedPoints", { totalEarnedPoints: wallet.totalEarnedPoints }).orWhere(
    ...
    async updateLeaderboard(model: UpdateLeaderboardDto) {
        try {
            const { walletId, lastUpdateTime, lendingUSDCPoints, borrowingUSDCPoints, totalEarnedPoints, claimedPoints, referralBoost } = model;
    ...
    async remove(id: string): Promise<ResultDto<null>> {
        ...
        const message = new ResponseMessage("Wallet removed successfully", "Wallet removed successfully");
    ...
    > {
            ...
            .where(
                new Brackets((qb) => {
                    qb.where("w2.totalEarnedPoints > wallet.totalEarnedPoints").orWhere(
                        new Brackets((qb2) => {
                            qb2.where("w2.totalEarnedPoints = wallet.totalEarnedPoints").andWhere("w2.createdAt < wallet.createdAt");
                        })
                    );
                })
            );
        ...
        const totalPointsResult = await this.walletRepository.createQueryBuilder("wallet").select("SUM(wallet.totalEarnedPoints)", "sum").getRawOne();
