...
export class BlockService {
    ...
    async findMissingBlocks(latestOnlineBlock: number): Promise<number[]> {
        try {
            ...
            this.logger.log(`Finding missed blocks between ${latestCheckedBlock.blockNumber + 1} and ${latestOnlineBlock - 1} count: ${latestOnlineBlock - latestCheckedBlock.blockNumber}`);
            ...
            const missedBlockNumbers = Array.from({ length: latestOnlineBlock - latestCheckedBlock.blockNumber - 1 }, (_, i) => i + latestCheckedBlock.blockNumber + 1).filter(
                (blockNumber) => !existingBlockNumbers.has(blockNumber)
            );
